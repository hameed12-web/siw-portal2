...
# Pre-seed text storage maps on boot
load_data(TRAINEES_FILE)
load_data(CENTERS_FILE)
load_data(USERS_FILE)
load_data(DOCS_FILE)

# PATH-PROOFER RESOLVER: Searches all nested sub-directories on GitHub to render files safely
def search_and_render(template_name, **context):
    possible_paths = [
        os.path.join(app.root_path, template_name),
        os.path.join(app.root_path, 'templates', template_name),
        os.path.join(app.root_path, 'templates', 'templates', template_name)
    ]
    ...
    if request.method == 'POST':
        ...
        for t in trainees:
            if t.get('cnic_number') == cnic:
                error_msg = "Submission Denied: This CNIC Number is already registered."
            if t.get('phone_number') == phone:
                error_msg = "Submission Denied: This Mobile Number is already registered."
            if vendor and t.get('vendor_id') == vendor:
                error_msg = "Submission Denied: This Vendor ID is already registered."
            if iban and t.get('iban') == iban:
                error_msg = "Submission Denied: This Bank Account Number (IBAN) is already registered."
            if error_msg: break
        ...
        return "<div style='color:green; font-family:sans-serif; text-align:center; padding:50px;'><h2>Registration Submitted with Unique Constraints Check Verified!</h2><br><a href='/'>Go Back Home</a></div>"
        
    all_centers = load_data(CENTERS_FILE)
    functional_centers = [c for c in all_centers if c.get('status') == 'Functional']
    return search_and_render('public_form.html', centers=functional_centers)
    ...
    if request.method == 'POST':
        ...
        for u in users:
            if u.get('username') == username and u.get('password') == password:
                ...
                return redirect(url_for('admin_dashboard'))
    return search_and_render('admin_login.html')
...
def admin_dashboard():
    ...
    return search_and_render('dashboard.html', trainees=view_trainees, centers=view_centers, extra_keys=list(extra_keys), total_trainees=total_trainees, trade_metrics=trade_metrics, ddo_accounts=ddo_accounts, all_centers_list=all_centers, docs_to_me=docs_to_me, docs_from_me=docs_from_me)
